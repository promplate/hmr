...
MARKER = "HMR_PROBE_XINFERENCE_CPU_PRINT_7C31A8E4_001"
...
def print_statement(marker: str = MARKER) -> str:
    ...
    ran" but "the new code ran in the sub-pool process that owns the loaded model".
    """
    if not marker.startswith("HMR_PROBE_XINFERENCE_") or not marker.replace("_", "").isalnum():
        raise ValueError("marker must be an alphanumeric HMR_PROBE_XINFERENCE_ token")
...
class MutationSet:
    ...
    def append_raw(self, relative: str, text: str) -> None:
        """Append raw text, used to inject a syntax error the preflight must reject."""
        path = self._save(relative)
