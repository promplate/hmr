...
def event_log_records(directory: Path, pid: int) -> list[dict[str, Any]]:
    """One process's own `HMR_VLLM_EVENT_LOG_DIR` file. A line still being appended is skipped, not fatal."""
    path = directory / f"hmr-events-{pid}.jsonl"
...
def wait_queued_out_of_band(directory: Path, pids: list[int], timeout: float = 120) -> dict[int, dict[str, Any]]:
    """Wait for every process's own watcher to queue the target, without touching the server.

    Out-of-band is the whole point. With the packaged middleware every HTTP request is a
    publication boundary, so polling the state endpoint here would itself be the boundary that
    publishes the edit, and the receipt would name that poll rather than the first real inference
    request. The packaged runtime's event file is readable while the server is untouched, so no
    interval has to be guessed either.
    """
    deadline = time.monotonic() + timeout
    ...
    while True:
        for pid in pids:
            if pid in queued:
                continue
            for record in event_log_records(directory, pid):
                if record.get("kind") == "source_change" and record.get("path") == TARGET:
                    queued[pid] = record
                    break
        if len(queued) == len(pids):
            return queued
        if time.monotonic() >= deadline:
            raise TimeoutError(f"not every process's watcher queued {TARGET} out of band: queued={sorted(queued)}, expected={sorted(pids)}")
        time.sleep(0.2)
...
def _run(args: argparse.Namespace) -> None:
    ...
    # The packaged runtime's out-of-band sink. Read directly, so waiting for the watcher to queue
    # the edit never sends an HTTP request, which with the packaged middleware would itself publish.
    event_log_dir = results / "cpu-hmr-events"
    for artifact in (receipt_path, log_path, manifest_path, event_log_dir):
        if artifact.exists():
    ...
    for key in ("HMR_VLLM_RUNTIME", "HMR_VLLM_DISABLED", "HMR_VLLM_SKIP", "HMR_VLLM_ENABLE"):
        env.pop(key, None)
    # `vllm-hmr` sets HMR_VLLM_ENABLE, _RUNTIME, _SOURCE_ROOT and _MANIFEST itself. The one other
    # knob is the packaged runtime's documented event sink, which only writes; it installs no
    # watcher and publishes nothing. Everything else here is vLLM's own.
    env.update(
        {
            "HMR_VLLM_EVENT_LOG_DIR": str(event_log_dir),
    ...
    try:
        ...
        try:
            ...
            # Out of band, so nothing between the edit and the inference request below touches the
            # server: every HTTP request is a publication boundary for the packaged middleware, so
            # a state poll here would publish the edit and own the boundary the receipt records.
            # Every process is waited for, not just the API one, so the boundary below finds
            # something to decide in each worker instead of racing its watcher.
            queued_out_of_band = wait_queued_out_of_band(event_log_dir, [before_identity["api_pid"], *(worker["pid"] for worker in before_identity["workers"])])
            marker_line = f"{MARKER} pid={before_identity['api_pid']}"
            pre_request_log = log_path.read_text(encoding="utf-8", errors="replace")
            if marker_line in pre_request_log:
                raise AssertionError(f"the marker ran before the first post-edit request: {marker_line}")
            # The first HTTP request after the edit, and a real one: publication happens at its
            # boundary and the new code runs inside it.
            post_status, post_headers, post_payload = completion(base, args.model)
            if post_status != 200:
                raise AssertionError((post_status, post_payload))
            after = state(base)  # readback only, after the inference request has already published
            after_identity = identity(after)
            if after_identity != before_identity:
                raise AssertionError((before_identity, after_identity))
            found = publication_for_target(after)
            if found is None:
                raise AssertionError(f"the packaged runtime never published {TARGET}: pending={after['api']['pending']}")
            publication_boundary, publication = found
            if publication_boundary.get("path") != "/v1/completions":
                raise AssertionError(f"the target was published at {publication_boundary.get('path')!r}, not at the inference request")
            queued_event = queued_before(after, publication_boundary)
            if queued_event is None:
                raise AssertionError(f"the target was published with no preceding watcher event: {publication_boundary}")
            ...
            # Sliced at the pre-request length, not the pre-edit one: the marker must be attributable
            # to this one request, and the absence check above is what makes that slice meaningful.
            post_request_log = log_after[len(pre_request_log) :]
            if marker_line not in post_request_log:
                raise AssertionError(f"marker missing from correct process log: {marker_line}")
            load_evidence_after_edit = load_lines(log_after[len(before_log) :])
            ...
            receipt.update(
                {
                    ...
                    "out_of_band_queued_events": queued_out_of_band,
                    "out_of_band_event_log_dir": str(event_log_dir),
                    ...
                    "assertions": {
                        **receipt["assertions"],
                        ...
                        "every_watcher_queued_the_target_out_of_band_before_any_request": True,
                        "no_http_request_between_the_edit_and_the_first_real_inference": True,
                        ...
                        "published_at_the_real_completions_request_boundary": publication_boundary.get("path") == "/v1/completions",
                        "worker_publication_rpc_at_the_same_completions_boundary": worker_boundary.get("path") == "/v1/completions",
                        "marker_absent_before_the_first_post_edit_request": True,
