...
??? info "Why Python 3.12+?"
    ...
    ---
    For the same reason, [PyPy](https://github.com/pypy/pypy) is not supported as for now [latest PyPy](https://pypy.org/posts/2025/07/pypy-v7320-release.html) only supports up to Python 3.11.
...
- builds dependency graphs **at runtime**, thus supporting dependencies generated in various forms (e.g., `#!py exec("import ...")` or `#!py __import__("...")`), which cannot be statically analyzed and are widespread in the Python ecosystem
...
!!! note ""
    ...
    In contrast, **this project avoids monkey-patching**: it leverages Python's [`sys.meta_path`](https://docs.python.org/3/reference/import.html#the-meta-path) hooks and a custom [`ModuleType`](https://docs.python.org/3/library/types.html#types.ModuleType) to track dependencies.
...
- [`#!py importlib.reload`](https://docs.python.org/3/library/importlib.html#importlib.reload) in Python module mechanism, [`%autoreload`](https://ipython.readthedocs.io/en/stable/config/extensions/autoreload.html) in IPython and [reloadium](https://pypi.org/project/reloadium/) etc. all apply to the two points above
