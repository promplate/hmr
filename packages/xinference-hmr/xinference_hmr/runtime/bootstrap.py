"""Install pyth-on-line HMR in the process that owns a loaded model, and publish at actor boundaries.

This never touches model weights, the tokenizer, the loaded `torch.nn.Module`, the
`ModelActor` instance, or the model object's class. It only re-executes one Python
module on disk change, nothing else.

Publication is driven by `.actor`, which wraps `ModelActor.__on_receive__`. That is
the actor's own request boundary inside the sub-pool process -- not the REST layer
...
def max_watcher_restarts() -> int:
    """How many times a dead watcher may be restarted, overridable via `HMR_XINFERENCE_MAX_WATCHER_RESTARTS`.
    ...
    """
    raw = os.getenv("HMR_XINFERENCE_MAX_WATCHER_RESTARTS")
    if not raw:
...
def install_from_env() -> None:
    ...
    with _INSTALL_LOCK:
        if _INSTALLED:
            return
        source_root_raw = os.environ.get("HMR_XINFERENCE_SOURCE_ROOT")
        if not source_root_raw:
            raise RuntimeError("HMR_XINFERENCE_SOURCE_ROOT is required but not set")
        source_root = Path(source_root_raw).resolve()
        manifest_path_raw = os.getenv("HMR_XINFERENCE_MANIFEST")
        ...
        event("hmr_installed", pid=os.getpid(), includes=include_paths, manifest_paths=list(manifest.reactive_paths))
...
def _start_watcher() -> None:
    ...
    _WATCH_STOP = threading.Event()
    _WATCH_THREAD = threading.Thread(target=_watch, name="xinference-hmr-watch", daemon=True)
    _WATCH_THREAD.start()
...
def _recover_watcher() -> dict[str, Any]:
    ...
    Called from the actor boundary, never from the watcher thread: the thread that died cannot
...
def _after_fork() -> None:
    ...
    Without this, a forked child reports HMR as installed while nothing is ever queued (its
    watcher does not exist), and any lock a parent thread held at fork time stays locked
    forever. Xinference's local deploy sets the `spawn` start method, where this never runs.
    """
...
def _watch() -> None:
    ...
    try:
        ...
        for changes in watch(
            *watch_paths,
            debounce=int(os.getenv("HMR_XINFERENCE_DEBOUNCE_MS", "300")),
            step=50,
...
def _restore_module_state(module, snapshot: _ModuleSnapshot) -> None:
    ...
    # `AttributeError` for a name that only exists in the namespace.
    ...
    for key in [key for key in namespace if key not in snapshot.namespace and key != "_ReactiveModule__load"]:
        ...
        if signal is not None and signal._value:  # noqa: SLF001
            ...
            # `ReactiveMappingProxy.__delitem__` raises `KeyError` for it. Dropping it from the raw
            # dict restores the namespace without inventing a signal transition no subscriber saw.
            del namespace[key]
    for key, value in snapshot.namespace.items():
        ...
        if key not in namespace or namespace[key] is not value or (signal is not None and not signal._value):  # noqa: SLF001
            ...
            # caller through the proxy, keeping the per-name signal in step.
...
def sync_pending(*, force: bool = False) -> dict[str, Any]:
    """Publish queued changes at an actor request boundary. Never invoked by the watcher thread."""
    if not _INSTALLED:
    ...
    def result(*, hmr_available: bool = True, **extra: Any) -> dict[str, Any]:
        """One shape for every return below, so no exit can omit a field a caller reads."""
    ...
    if active_scopes() and not force:
        event("publication_deferred", reason="active_requests")
            ...
            with HMR_CONTEXT.batch():
                for path, record in items:
                    ...
                    if module is None:
                        ...
                        # Not being in the map means the live `xinference` came from somewhere else
                        # (a site-packages install while the source root is only a copy), so calling
                        # this "published" would claim a swap that never touched running code. This
                        # is a real hazard here: the sub-pool is started with `-m`, so its cwd wins
                        # `sys.path[0]` while the API process imports from site-packages.
...
def manifest_source_root() -> Path | None:
    """The root this process is actually watching, or `None` when HMR was never installed here."""
    manifest = _MANIFEST
    return None if manifest is None else manifest.source_root
