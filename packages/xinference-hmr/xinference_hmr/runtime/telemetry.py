...
The in-flight request count is not diagnostics, it is the safety interlock: a source
swap while a request is mid-flight would let one request observe two different
...
def reset_after_fork() -> None:
    ...
    _LOCK = threading.RLock()
    _ACTIVE_SCOPES = 0  # no in-flight actor call survives a fork, so nothing is in flight here
...
def snapshot() -> dict[str, Any]:
    with _LOCK:
        return {
            ...
            "events": list(_EVENTS),
            "source_root": os.getenv("HMR_XINFERENCE_SOURCE_ROOT"),
