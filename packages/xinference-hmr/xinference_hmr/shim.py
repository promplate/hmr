"""Importable, testable half of the `sitecustomize` shim used by `xinference-hmr`."""
...
SKIP_MARKER = "HMR_XINFERENCE_SKIP"  # set by a process that must not inherit injection
...
def parse_runtime(spec: str) -> tuple[str, str]:
    ...
    if not sep or not module or not attribute:
        raise ValueError(f"HMR_XINFERENCE_RUNTIME must be 'module:callable', got {spec!r}")
    return module, attribute


def should_install(env: dict[str, str]) -> bool:
    return env.get("HMR_XINFERENCE_ENABLE") == "1" and not env.get(SKIP_MARKER) and bool(env.get("HMR_XINFERENCE_RUNTIME"))
...
def load_runtime(spec: str):
    ...
    if not callable(target):
        raise ValueError(f"HMR_XINFERENCE_RUNTIME {spec!r} resolves to {type(target).__name__}, which is not callable")
    return target
...
def check_runtime(spec: str) -> None:
    ...
    `site` reports a failing `sitecustomize` as one line on stderr and carries on, which
    inside Xinference's startup output reads as "HMR silently did nothing". Same resolution
    as `load_runtime`, minus the call.
    """
    try:
        load_runtime(spec)
    except ValueError:
        ...
        raise ValueError(f"HMR_XINFERENCE_RUNTIME {spec!r} could not be loaded: {type(exc).__name__}: {exc}") from exc
...
def install_from_env(env: dict[str, str] | None = None) -> object | None:
    ...
    if not should_install(env):
        return None
    return load_runtime(env["HMR_XINFERENCE_RUNTIME"])()
