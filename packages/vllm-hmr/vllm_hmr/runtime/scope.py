...
def syntax_preflight(path: Path) -> tuple[bool, str | None]:
    """Compile without executing so every syntax error is caught before the HMR loader."""
    try:
        compile(path.read_text(encoding="utf-8"), str(path), "exec", dont_inherit=True)
    except (OSError, SyntaxError, UnicodeError, ValueError) as exc:  # ValueError covers an embedded NUL byte
