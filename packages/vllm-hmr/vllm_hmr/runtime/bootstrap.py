...
def _first_uncompilable_dependent(names, by_name: dict[str, Any]) -> tuple[str, str] | None:
    """The first forced dependent whose file will not compile, as `(name, error)`, or `None`.

    Read through `object.__getattribute__`: `__file__` is in the reactive namespace, so a plain
    attribute access would subscribe this call to it and re-execute the module we are checking.
    A dependent with no `__file__` (namespace package, exotic loader) is not judged here — it has
    no file to preflight, and the transaction below is what decides it.
    """
    for name in names:
        ...
        if dependent is None:
            continue
        try:
            file = object.__getattribute__(dependent, "__file__")
        except AttributeError:
            continue
        if file is None or not str(file).endswith(".py"):
            continue
        ok, error = syntax_preflight(Path(file))
        if not ok:
            ...
            with HMR_CONTEXT.batch():
                for path, record in items:
                    ...
                    if module is None:
                        ...
                        continue
                    load = _load_handle(module)
                    # Preflight every forced dependent before touching the target. `hmr`'s loader
                    # reports a dependent's SyntaxError to `sys.excepthook` and returns normally, so
                    # `dependent_load()` below raises nothing and the target would be published
                    # against a dependent still holding the old function object — a version split
                    # inside one process. A dependent that is not loaded at all is not decided here:
                    # that stays inside the transaction, where it already rolls the target back.
                    broken = _first_uncompilable_dependent(_MANIFEST.forced_dependents.get(rel, ()), by_name)
                    if broken is not None:
                        rejected.append({**record, "error": f"forced dependent {broken[0]} would not compile: {broken[1]}"})
                        settled.add(path)
                        continue
