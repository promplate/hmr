...
The default scope is deliberately two files. `python/sglang/srt/model_executor/forward_context.py`
is the provider that the CPU smoke actually hot-replaces; `python/sglang/srt/model_executor/model_runner.py`
...
TARGET = "python/sglang/srt/model_executor/forward_context.py"
DEPENDENT = "sglang.srt.model_executor.model_runner"
DEPENDENT_PATH = "python/sglang/srt/model_executor/model_runner.py"
...
# The scope a manifest must name exactly, in the same shape `Manifest.declared_scope` returns.
DECLARED_SCOPE: dict[str, object] = {
    "files": sorted(REACTIVE_PATHS),
    "reactive_paths": sorted(REACTIVE_PATHS),
    "auto_paths": sorted(AUTO_PATHS),
    "forced_dependents": {key: sorted(value) for key, value in FORCED_DEPENDENTS.items()},
}
...
def validate_source_root(root: Path) -> Path:
    ...
    if not target.is_file():
        raise ScopeError(f"source root {resolved} does not contain {TARGET}; pass --hmr-source-root pointing at a SGLang source tree")
...
def build_manifest(root: Path) -> Manifest:
    ...
    return Manifest(source_root, files, REACTIVE_PATHS, AUTO_PATHS, FORCED_DEPENDENTS)
...
def load_manifest(path: Path, expected_root: Path | None = None) -> Manifest:
    ...
    # `type(...) is int`, not `== 1`: JSON `true` and `1.0` both compare equal to 1, and a
    # manifest whose version is a bool or a float does not state a version this runtime knows.
    version = raw.get("schema_version")
    if type(version) is not int or version != MANIFEST_SCHEMA_VERSION:
        raise ScopeError(f"manifest {path} has schema_version {version!r}, expected {MANIFEST_SCHEMA_VERSION}")
...
def verify_manifest(manifest: Manifest) -> None:
    ...
    The expected scope is a constant, so it is compared directly rather than by re-deriving a
    whole `Manifest` — that would hash every watched file a second time to learn nothing.
    """
    if manifest.declared_scope() != DECLARED_SCOPE:
        raise ScopeError(f"manifest does not match this runtime's verified scope exactly: {manifest.declared_scope()} != {DECLARED_SCOPE}")
