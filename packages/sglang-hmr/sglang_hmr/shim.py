"""Importable, testable half of the `sitecustomize` shim used by `sglang-hmr`."""
...
def parse_runtime(spec: str) -> tuple[str, str]:
    ...
    if not sep or not module or not attribute:
        raise ValueError(f"HMR_SGLANG_RUNTIME must be 'module:callable', got {spec!r}")
    return module, attribute


def should_install(env: dict[str, str]) -> bool:
    return env.get("HMR_SGLANG_ENABLE") == "1" and not env.get(SKIP_MARKER) and bool(env.get("HMR_SGLANG_RUNTIME"))
...
def load_runtime(spec: str):
    ...
    if not callable(target):
        raise TypeError(f"HMR_SGLANG_RUNTIME {spec!r} resolves to {type(target).__name__}, which is not callable")
    return target
...
def check_runtime(spec: str) -> None:
    ...
    try:
        ...
        raise ValueError(f"HMR_SGLANG_RUNTIME {spec!r} could not be loaded: {type(exc).__name__}: {exc}") from exc
...
def install_from_env(env: dict[str, str] | None = None) -> object | None:
    ...
    if not should_install(env):
        return None
    return load_runtime(env["HMR_SGLANG_RUNTIME"])()
